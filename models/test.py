...
def test_img_wm(net_g, datatest, args, test_backdoor=False):
    ...
    for idx, (data, target) in enumerate(data_loader):
        ...
        if test_backdoor:
            del_arr = []
            for k, image in enumerate(data):
                if test_or_not_wm(args, target[k]):  # one2one need test
                    # data[k][:, 0:5, 0:5] = torch.max(data[k])
...
def test_or_not_wm(args, label):
    if args.wm_goal != -1:  # one to one
        ...
        if label != args.wm_label:
            return True
        else:
            return False
        
def add_trigger_wm(args, image):
        if args.wm_type == 'dba':
            pixel_max = 1
            image[:,args.wm_triggerY+0:args.wm_triggerY+2,args.wm_triggerX+0:args.wm_triggerX+2] = pixel_max
            image[:,args.wm_triggerY+0:args.wm_triggerY+2,args.wm_triggerX+2:args.wm_triggerX+5] = pixel_max
            image[:,args.wm_triggerY+2:args.wm_triggerY+5,args.wm_triggerX+0:args.wm_triggerX+2] = pixel_max
            image[:,args.wm_triggerY+2:args.wm_triggerY+5,args.wm_triggerX+2:args.wm_triggerX+5] = pixel_max
            save_img_wm(image)
            return image
        if args.wm_type == 'square':
            pixel_max = torch.max(image) if torch.max(image)>1 else 1
            
            image[:,args.wm_triggerY:args.wm_triggerY+5,args.wm_triggerX:args.wm_triggerX+5] = pixel_max
        elif args.wm_type == 'pattern':
            pixel_max = torch.max(image) if torch.max(image)>1 else 1
            image[:,args.wm_triggerY+0,args.wm_triggerX+0] = pixel_max
